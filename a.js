...
let getProduct = async datas => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			tempData = [{ sku: 'WDTA50SAKZ', original_sku: 'WDTA50SAKZ', brand: 'Whirlpool' }]
...
const setLowesStore = async page => {
	try {
		...
		// await page.type('input[placeholder="Zip Code, City, State or Store #"]', '63011', {
		// 	delay: 100
		// })
		// await page.waitForTimeout(200)
			...
			try {
				...
				if (element != null) {
					...
					if (checkModel.includes(item.sku.toUpperCase()) || checkModel.includes(item.original_sku)) {
						...
						let oosData = "#sticky-container > div.style__Wrapper-PDP__sc-18tuvx0-0.gIkGaM > div.style__ProductDescDeskWrapper-PDP__sc-1a0l1ro-2.IVbhy > div.BackInStockstyles__BackInStockWrapper-sc-zpzi55-0.dSBqYy.backInStockSlider > div > div.warnInfo.gd-info > div.warnMessage > span"
						if (await page.$(oosData) != undefined && await page.$(oosData) != null) {
							if (await page.evaluate(el => el.textContent, await page.$(oosData)) != undefined && await page.evaluate(el => el.textContent, await page.$(oosData)) != '') {
								in_stock_status = 0
							}
						}

						// console.log(await page.evaluate(el => el.textContent, await page.$(oosData)))
						console.log(await page.evaluate(el => el.textContent, await page.$('#headerApp > div:nth-child(2) > div > div.sc-105kpm3-10.ikjBHq > header > div.sc-105kpm3-2.kngyuR > div > div.sc-1ecnx9w-5.erLXxh > div > a > span > span')))
